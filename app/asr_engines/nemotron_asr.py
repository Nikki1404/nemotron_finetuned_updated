...
class NemotronStreamingASR(ASREngine):
    ...
    def __init__(self, model_name: str, device: str, sample_rate: int):
        ...
        self.end_silence_ms = int(os.getenv("NEMO_END_SILENCE_MS", "900"))
        self.min_utt_ms = int(os.getenv("NEMO_MIN_UTT_MS", "200"))
        self.finalize_pad_ms = int(os.getenv("FINALIZE_PAD_MS", "800"))
    ...
    def _to_device(self, x: torch.Tensor) -> torch.Tensor:
        return x.cuda(non_blocking=True) if self.device == "cuda" else x.cpu()
    ...
    def set_language(self, language: str):
        ...
        try:
            ...
            import logging
            logging.getLogger(__name__).warning(f"set_inference_prompt('{language}') failed: {e}")

    def load(self) -> float:
        import nemo.collections.asr as nemo_asr
        t0 = time.time()
        if self.model_name.endswith(".nemo"):
            self.model = nemo_asr.models.EncDecRNNTBPEModelWithPrompt.restore_from(self.model_name, map_location="cpu")
        else:
            self.model = nemo_asr.models.EncDecRNNTBPEModelWithPrompt.from_pretrained(self.model_name, map_location="cpu")
        ...
        try:
            ...
            self.model.encoder.set_default_att_context_size((70, int(self.context_right)))
        self.model.change_decoding_strategy(decoding_cfg=OmegaConf.create({"strategy":"greedy","greedy":{"max_symbols":self.max_symbols,"loop_labels":False,"use_cuda_graph_decoder":False}}))
        self.model.eval()
        try:
    ...
    def new_session(self, max_buffer_ms: int):
        return StreamingRNNTSession(self, max_buffer_ms=max_buffer_ms)

    @torch.inference_mode()
    def stream_transcribe(self, audio_f32: np.ndarray, cache: Tuple[torch.Tensor, torch.Tensor, torch.Tensor], prev_hyp: Any, prev_pred_out: Any, emitted_frames: int, force_flush: bool = False):
        ...
        if not enough and not force_flush:
            return None, cache, prev_hyp, prev_pred_out, emitted_frames, timings
        if emitted_frames == 0:
            chunk_start = 0; chunk_end = min(self.shift_frames, available); drop_extra = 0
        else:
        ...
        (prev_pred_out, texts, cache0, cache1, cache2, prev_hyp) = self.model.conformer_stream_step(processed_signal=chunk_mel, processed_signal_length=chunk_len, cache_last_channel=cache[0], cache_last_time=cache[1], cache_last_channel_len=cache[2], keep_all_outputs=False, previous_hypotheses=prev_hyp, previous_pred_out=prev_pred_out, drop_extra_pre_encoded=drop_extra, return_transcription=True)
...
class StreamingRNNTSession:
    ...
    def reset_stream_state(self):
        ...
        self.prev_hyp = None; self.prev_pred = None; self.emitted_frames = 0
        self.current_text = ""; self.audio = np.array([], dtype=np.float32)
        self.utt_preproc = 0.0; self.utt_infer = 0.0; self.utt_flush = 0.0; self.chunks = 0
        self._trimmed_since_last_step = False
    ...
    def _is_new_text(self, new_text):
        if new_text is None: return False
        new = new_text.strip(); old = self.current_text.strip()
        if new == "" or new == old: return False
        if new.startswith(old): return True
        if old.startswith(new): return False
        return True
    ...
    def step_if_ready(self):
        if self._trimmed_since_last_step and self.emitted_frames > 0:
            cache = self.engine.model.encoder.get_initial_cache_state(batch_size=1)
            self.cache = self.engine._move_cache_to_device((cache[0], cache[1], cache[2]))
            self.prev_hyp = None; self.prev_pred = None; self.emitted_frames = 0; self._trimmed_since_last_step = False
        text, self.cache, self.prev_hyp, self.prev_pred, self.emitted_frames, t = self.engine.stream_transcribe(audio_f32=self.audio, cache=self.cache, prev_hyp=self.prev_hyp, prev_pred_out=self.prev_pred, emitted_frames=self.emitted_frames, force_flush=False)
        self.utt_preproc += t.preproc_sec; self.utt_infer += t.infer_sec
        if not self._is_new_text(text): return None
        self.current_text = text.strip(); self.chunks += 1
    ...
    def finalize(self, pad_ms: int) -> str:
        ...
        t0 = time.perf_counter()
        text, self.cache, self.prev_hyp, self.prev_pred, self.emitted_frames, t = self.engine.stream_transcribe(audio_f32=self.audio, cache=self.cache, prev_hyp=self.prev_hyp, prev_pred_out=self.prev_pred, emitted_frames=self.emitted_frames, force_flush=True)
        self.utt_preproc += t.preproc_sec; self.utt_infer += t.infer_sec; self.utt_flush += time.perf_counter() - t0
        if text: self.current_text = text.strip()
